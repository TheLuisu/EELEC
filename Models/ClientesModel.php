...
class ClientesModel extends Query{
    ...
    public function registroDirecto($nombre, $correo, $clave, $token){
        ...
        $sql = "INSERT INTO clientes (nombre_cliente, correo_cliente, password_cliente, token) VALUES (?,?,?,?)";
    ...
    public function getVerificar($correo){
        $sql = "SELECT * FROM clientes WHERE correo_cliente = '$correo'";
        return $this->select($sql);
    }

    public function registrarPedido($id_transaccion, $monto_pedido, $estado, $fecha, $email, $nombre, $apellido, $direccion, $ciudad, $email_user){
        $sql = "INSERT INTO pedidos (id_transaccion, monto, estado, fecha, email, nombre, apellido, direccion, ciudad, email_user) VALUES (?,?,?,?,?,?,?,?,?,?)";
        $datos = array($id_transaccion, $monto_pedido, $estado, $fecha, $email, $nombre, $apellido, $direccion, $ciudad, $email_user);
        $data = $this->insertar($sql, $datos);
        if ($data > 0){
            $res = $data;
        } else {
